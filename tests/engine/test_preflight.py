...
def make_ctx(**over):
    base = dict(is_admin=True, dest_reachable=True, volume_backed=True,
                ...
                quota_ok=True, dot_readable=True, dot_is_true=False, dot_flippable=True)
    base.update(over)
    return MigrationContext(**base)


def test_p9_dot_capability():
    assert next(c for c in run_preflight(make_ctx()) if c.id == "P9").passed is True
    bad = make_ctx(dot_readable=True, dot_is_true=True, dot_flippable=False)
    assert next(c for c in run_preflight(bad) if c.id == "P9").passed is False
